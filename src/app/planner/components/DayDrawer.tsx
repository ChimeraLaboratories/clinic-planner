...
import type { Session, Clinician, DayRule } from "../types/planner";
...
export default function DayDrawer({
    ...
    // ✅ optional so existing usages don’t break
    ...
    const showExpectedSidebar =
        !!clinicians && !!dayRules && !!trainingStart;
    ...
    return (
        <div className="fixed inset-0 z-50">
            ...
            {/* Right-side panel(s) */}
            <div className="absolute right-0 top-0 h-full flex shadow-xl border-l border-slate-200 bg-white">
                {/* Drawer */}
                            ...
                            <div className="space-y-2">
                                {sessions.map((s) => {
                                    ...
                                    const clinicianId = Number(
                                        (s as any).clinician_id
                                    );

                                    const roomName =
                                        roomsById.get(roomId) ?? `Room ${roomId}`;
                                    const clinicianName =
                                        cliniciansById.get(clinicianId) ?? "";
            ...
            {createOpen && createDefaults && (
                <CreateSessionModal
                    rooms={Array.from(roomsById.entries()).map(([id, name]) => ({
                        id,
                        name,
                    }))}
                    clinicians={Array.from(cliniciansById.entries()).map(
                        ([id, display_name]) => ({
                            id,
                            display_name,
                        })
